...
class Blocks:
    ...
    def IMAGE(self, img_url: str, alt_txt: str = "", text: str = "") -> dict:
        block: dict = {"type": "image", "image_url": img_url, "alt_text": alt_txt}

        if text != "":
            block["title"] = {"type": "plain_text", "text": text, "emoji": True}
        return block
...
class Slack:
    def __init__(self):
        self.client = WebClient(token=get_password("Slack-pythonbot", "token"))
        self.BLOCK = Blocks()
        return
    ...
    def channel_id(self, channel_name: str) -> str:
        ...
        return get_password("Slack-pythonbot", channel_name)
    ...
    def msg(self, message: str, channel: str = "python") -> int:
        ...
        try:
            _ = self.client.chat_postMessage(
                channel=self.channel_id(channel), text=message
            )
    ...
    def post_block(self, channel: str, blocks: list[dict]):
        ...
        try:
            _ = self.client.chat_postMessage(
                channel=self.channel_id(channel), blocks=blocks
            )
        except SlackApiError as e:
            # You will get a SlackApiError if "ok" is False
            print(f"NG - Slack message not sent: {str(e)}")
